...
app.put("/admin/areas/:areaId/rejeitar", async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      ...
      const area = await db.buscarUm(
        "SELECT * FROM areas_responsabilidade WHERE id = ?",
        [areaId]
      );
      if (area && area.ong_id) {
        ...
        await db.inserir(
          ...
          [
            ...
            "Área rejeitada",
            `Sua área "${area.nome}" foi rejeitada pelo administrador.${
              motivo_rejeicao ? " Motivo: " + motivo_rejeicao : ""
            }`,
          ]
        );
        // GET /usuarios - retorna todos os usuários (ONGs e outros)
    ...
    res
      .status(500)
      .json({ error: "Erro ao rejeitar área", details: error.message });
  }
});
...
app.delete("/areas/:id", autenticarONGouAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!req.isAdmin && area.ong_id != req.usuarioId) {
      console.error(
        `[EXCLUIR ÁREA] ONG não autorizada. ONG da área: ${area.ong_id}, Usuário autenticado: ${req.usuarioId}`
      );
      return res
        .status(403)
        .json({ error: "Você só pode excluir áreas da sua própria ONG." });
    }
    ...
    if (
      ...
      console.error(
        `[EXCLUIR ÁREA] Status inválido para exclusão. Status: ${area.status}, ID: ${id}`
      );
      return res
        .status(400)
        .json({
          error:
            "Só é possível excluir áreas pendentes, aprovadas ou rejeitadas.",
        });
    }
    // Não criar notificação aqui! Apenas excluir.
    await db.executarUpdate("DELETE FROM areas_responsabilidade WHERE id = ?", [
      id,
    ]);
...
app.put("/denuncias/:id/rejeitar", async (req, res) => {
  ...
  try {
    console.log(
      `[DENUNCIA] Tentando rejeitar denúncia id=${id} com observacoes:`,
      observacoes
    );
    const result = await db.executarUpdate(
    ...
    if (result.affectedRows > 0) {
      console.log(`[DENUNCIA] Denúncia id=${id} rejeitada com sucesso.`);
      res.json({ success: true, id });
    } else {
      console.warn(
        `[DENUNCIA] Denúncia id=${id} não encontrada para rejeição.`
      );
      res.status(404).json({ error: "Denúncia não encontrada", id });
    }
  } catch (error) {
    console.error(
      `[DENUNCIA] Erro ao rejeitar denúncia id=${id}:`,
      error.message
    );
    res
      .status(500)
      .json({ error: "Erro ao rejeitar denúncia", details: error.message, id });
  }
});
...
app.get("/denuncias", async (req, res) => {
  try {
    ...
    let denuncias;
    if (req.query.ong_id) {
      denuncias = await db.executarQuery(
        "SELECT * FROM denuncias WHERE ong_responsavel_id = ?",
        [req.query.ong_id]
      );
    } else {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar denúncias", details: error.message });
  }
});
...
app.put("/notificacoes/:id/lida", async (req, res) => {
  ...
  try {
    const result = await db.executarUpdate(
      "UPDATE notificacoes_ong SET lida = 1 WHERE id = ?",
      [id]
    );
    if (result.affectedRows > 0) {
      res.json({ success: true, id });
...
app.get("/notificacoes", async (req, res) => {
  try {
    ...
    let notificacoes;
    if (ongId) {
      notificacoes = await db.executarQuery(
        "SELECT * FROM notificacoes_ong WHERE ong_id = ?",
        [ongId]
      );
    } else {
...
app.put("/admin/areas/:areaId/aprovar", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao aprovar área", details: error.message });
  }
});
...
app.get("/admin/areas/pendentes", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Erro ao buscar áreas pendentes",
        details: error.message,
      });
  }
});
...
app.get("/admin/areas", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar todas as áreas", details: error.message });
  }
});
...
app.get("/areas", async (req, res) => {
  try {
    let areas;
    if (req.query.ong_id) {
      areas = await db.executarQuery(
        "SELECT * FROM areas_responsabilidade WHERE ong_id = ?",
        [req.query.ong_id]
      );
    } else {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar áreas", details: error.message });
  }
});
...
app.post("/areas", async (req, res) => {
  try {
    ...
    if (!ong_id || !nome || !coordenadas) {
      return res
        .status(400)
        .json({ error: "Campos obrigatórios: ong_id, nome, coordenadas" });
    }
    ...
    res
      .status(500)
      .json({ error: "Erro ao criar área", details: error.message });
  }
});
...
app.delete("/lugares/:id", async (req, res) => {
  ...
  try {
    ...
    const marcador = await db.buscarUm("SELECT * FROM lugares WHERE id = ?", [
      id,
    ]);
    if (!marcador) {
      ...
      imagens.forEach((imgPath) => {
        if (typeof imgPath === "string" && imgPath.startsWith("/uploads/")) {
          ...
          fs.unlink(absPath, (err) => {
            if (err)
              console.error("Erro ao remover imagem:", absPath, err.message);
          });
        }
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Erro ao deletar marcador", details: error.message });
  }
});
...
app.post("/upload", upload.array("images", 10), (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao enviar imagens", details: error.message });
  }
});
  ...
  try {
    ...
    if (!nome || !tipo || latitude === undefined || longitude === undefined) {
      return res
        .status(400)
        .json({
          error: "Campos obrigatórios: nome, tipo, latitude, longitude",
        });
    }
    ...
    if (Array.isArray(imagem)) {
      imagensArray = imagem
        .map((img) => (typeof img === "object" && img.path ? img.path : img))
        .filter(Boolean);
        ...
        if (Array.isArray(coords) && coords.length >= 3) {
          ...
          const pontoNoPoligono = (lat, lng, poligono) => {
            ...
            for (
              ...
              const xi = parseFloat(poligono[i].lat),
                yi = parseFloat(poligono[i].lng);
              const xj = parseFloat(poligono[j].lat),
                yj = parseFloat(poligono[j].lng);
              const intersect =
                yi > lng !== yj > lng &&
    ...
    res
      .status(500)
      .json({ error: "Erro ao criar lugar", details: error.message });
  }
});
...
app.get("/lugares", async (req, res) => {
  try {
    ...
    const adaptados = lugares.map((lugar) => ({
      ...lugar,
      latitude:
        lugar.latitude !== undefined && lugar.latitude !== null
          ? Number(lugar.latitude)
          : null,
      longitude:
        lugar.longitude !== undefined && lugar.longitude !== null
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar lugares", details: error.message });
  }
});
...
app.put("/lugares/:id/resolver", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao atualizar marcador", details: error.message });
  }
});
app.get("/areas/publicas", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar áreas públicas", details: error.message });
  }
});
  ...
  try {
    ...
    if (!marcador_id || !motivo || !denunciante_id) {
      return res
        .status(400)
        .json({
          error: "Campos obrigatórios: marcador_id, motivo, denunciante_id",
        });
    }
    ...
    // Buscar ONG responsável (garantir preenchimento)
    let ong_responsavel_id = null;
    if (marcador.area_ong_id) {
      ong_responsavel_id = marcador.area_ong_id;
    } else {
      // Fallback: tentar buscar área pelo ponto do marcador
      const areas = await db.executarQuery(
        `SELECT * FROM areas_responsabilidade WHERE status = 'aprovada'`
      );
      for (const area of areas) {
        try {
          const coords = JSON.parse(area.coordenadas);
          if (Array.isArray(coords) && coords.length >= 3) {
            const pontoNoPoligono = (lat, lng, poligono) => {
              ...
              for (
                ...
                const xj = parseFloat(poligono[j].lat),
                  yj = parseFloat(poligono[j].lng);
                const intersect =
                  yi > marcador.longitude !== yj > marcador.longitude &&
                  marcador.latitude <
                    ((xj - xi) * (marcador.longitude - yi)) /
                      (yj - yi + 0.0000001) +
                      xi;
                if (intersect) dentro = !dentro;
              }
            ...
            if (
              ...
              ong_responsavel_id = area.ong_id;
              break;
            }
          }
        } catch (e) {
    ...
    // Log para depuração
    console.log(
      "[DENUNCIA] Criando denúncia para marcador",
      marcador_id,
      "ONG responsável:",
      ong_responsavel_id
    );
    ...
    res.json({ success: true, denuncia_id: result, ong_responsavel_id });
  } catch (error) {
    // Erro ao registrar denúncia
    res
      .status(500)
      .json({ error: "Erro ao registrar denúncia", details: error.message });
  }
});
