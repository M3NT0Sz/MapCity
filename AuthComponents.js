  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (response.ok && data.token && data.usuario) {
        ...
        setDocumentoValido(null);
        onClose();
      } else if (response.ok && data.requer_verificacao) {
        // Caso ONG precise de verificação
        Alert.alert('Cadastro realizado', 'Sua ONG foi cadastrada e está aguardando verificação do documento pelos administradores. Você poderá fazer login após a aprovação.');
        setNome('');
        setEmail('');
        setSenha('');
        setConfirmarSenha('');
